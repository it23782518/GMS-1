...
import { getTicketsRaisedByMember, getTicketsRaisedByStaff } from '../../services/ticketApi';
...
const TicketsByRaiserPage = () => {
  const [searchType, setSearchType] = useState('member'); // 'member' or 'staff'
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType === 'member') {
        response = await getTicketsRaisedByMember(searchId);
      } else {
        response = await getTicketsRaisedByStaff(searchId);
      }
      ...
      const transformedData = Array.isArray(response.data) ? 
        response.data.map(item => ({
          ...
          raisedByName: searchType === 'member' && item.member ? item.member.name : 
                       searchType === 'staff' && item.staff ? item.staff.name : 'Unknown',
          raisedById: searchType === 'member' && item.member ? item.member.id : 
                     searchType === 'staff' && item.staff ? item.staff.id : 'Unknown',
          raisedByType: searchType === 'member' ? 'MEMBER' : 'STAFF'
        })) : [];

      setTickets(transformedData);
      if (transformedData.length === 0) {
        setError(`No tickets found for ${searchType} ID: ${searchId}`);
      }
    } catch (err) {
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-4 mb-4">
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="member-search" className="ml-2 block text-sm text-gray-700">
                    Search by Member ID
                  </label>
                </div>
                <div className="flex items-center">
                  <input
                    ...
                    className="h-4 w-4 text-rose-600 focus:ring-rose-500 border-gray-300"
                  />
                  <label htmlFor="staff-search" className="ml-2 block text-sm text-gray-700">
                    Search by Staff ID
                  </label>
                </div>
              </div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-rose-700">
                      Showing tickets raised by {searchType === 'member' ? 'member' : 'staff'}: 
                      <span className="font-semibold ml-1">
                        {tickets[0].raisedByName} (ID: {tickets[0].raisedById})
                      </span>
                    </p>
                  </div>
                </div>
          ...
          {!loading && searched && tickets.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-500 mb-4 text-center">
                There are no tickets raised by this {searchType} with ID {searchId}.
              </p>
            </div>
          )}
        </div>
